...
export default function GestoresList() {
  const navigate = useNavigate();
  const [searchTerm, setSearchTerm] = useState('');
  const [searchInput, setSearchInput] = useState('');

  const baseWhere = searchTerm ? { nome: { contains: searchTerm, mode: 'insensitive' as const } } : undefined;
  const { data, loading, error, total, refetch, setSkip } = useList({
    queryName: 'gestores',
    fields: 'id nome cargo descricao foto { id url } areaResponsavel { id nome }',
    baseWhere, take: 10, skip: 0,
  });
  const items = (data as GestorItem[]) ?? [];
  ...
  const handleDelete = useCallback(async (item: GestorItem) => {
    if (!window.confirm(`Excluir "${item.nome}"?`)) return;
    try { await graphqlRequest(`mutation { deleteGestor(where: { id: "${item.id}" }) { id } }`); refetch(); }
    catch { alert('Erro ao excluir'); }
  }, [refetch]);

  return (
    <Layout title="Gestores">
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <p className="text-sm text-gray-500">Total de {total} gestor(es)</p>
          <button onClick={() => navigate('/gestores/novo')} className="inline-flex items-center gap-2 px-4 py-2 bg-[#003087] text-white font-medium rounded-lg hover:bg-[#001f54] text-sm">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" /></svg> Novo Gestor
          </button>
        </div>
        <form onSubmit={doSearch} className="flex gap-2">
          <input type="text" value={searchInput} onChange={(e) => setSearchInput(e.target.value)} placeholder="Buscar por nome..." className="flex-1 px-4 py-2 border border-gray-300 rounded-lg text-sm outline-none focus:ring-2 focus:ring-[#003087]" />
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-200">
                    <th className="text-left px-4 py-3 text-sm font-semibold text-gray-700">Foto</th>
                    <th className="text-left px-4 py-3 text-sm font-semibold text-gray-700">Nome</th>
                    <th className="text-left px-4 py-3 text-sm font-semibold text-gray-700">Cargo</th>
                ...
                <tbody>
                  {items.length === 0 ? (
                    <tr><td colSpan={5} className="px-4 py-8 text-center text-gray-500 text-sm">Nenhum gestor encontrado.</td></tr>
                  ) : (
                    items.map((item) => (
                      <tr key={item.id} className="border-b border-gray-100 hover:bg-gray-50">
                        <td className="px-4 py-2">
                          {item.foto?.url ? (
                            <img src={item.foto.url} alt={item.nome} className="w-10 h-10 rounded-full object-cover" />
                          ) : (
                        ...
                        <td className="px-4 py-3"><p className="text-sm font-medium text-gray-900">{item.nome}</p></td>
                        <td className="px-4 py-3 text-sm text-gray-600">{item.cargo}</td>
                        <td className="px-4 py-3 text-sm text-gray-600">{item.areaResponsavel?.nome ?? '-'}</td>
                        <td className="px-4 py-3 text-right">
                          <div className="flex justify-end gap-2">
                            <button onClick={() => navigate(`/gestores/${item.id}`)} className="px-3 py-1.5 text-xs font-medium text-[#003087] bg-blue-50 rounded-lg hover:bg-blue-100">Editar</button>
                            <button onClick={() => handleDelete(item)} className="px-3 py-1.5 text-xs font-medium text-red-700 bg-red-50 rounded-lg hover:bg-red-100">Excluir</button>
                          </div>
                        </td>
                      </tr>
                    ))
