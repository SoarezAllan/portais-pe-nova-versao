...
const categoriaColors: Record<string, string> = {
  ...
  normativa_interna: 'bg-orange-100 text-orange-800',
};

export default function LegislacaoList() {
  ...
  const { data, loading, error, total, refetch, setSkip } = useList({
    ...
    fields: 'id titulo linkExterno categoria resumo areaResponsavel { id nome } subarea { id nome }',
    baseWhere,
    take: 10,
    skip: 0,
  });

  const items = (data as LegislacaoItem[]) ?? [];
  ...
  const handleDelete = useCallback(async (item: LegislacaoItem) => {
    if (!window.confirm(`Excluir "${item.titulo}"?`)) return;
    try { await graphqlRequest(`mutation { deleteLegislacao(where: { id: "${item.id}" }) { id } }`); refetch(); } catch { alert('Erro ao excluir'); }
  }, [refetch]);

  return (
    <Layout title="Legislação">
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <p className="text-sm text-gray-500">Total de {total} legislação(ões)</p>
          <button onClick={() => navigate('/legislacao/novo')} className="inline-flex items-center gap-2 px-4 py-2 bg-[#003087] text-white font-medium rounded-lg hover:bg-[#001f54] text-sm">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" /></svg> Nova Legislação
          </button>
        </div>
          ...
          <>
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
              <table className="w-full">
                ...
                <tbody>
                  {items.length === 0 ? (
                    <tr><td colSpan={4} className="px-4 py-8 text-center text-gray-500 text-sm">Nenhuma legislação encontrada.</td></tr>
                  ) : (
                    items.map((item) => (
                      <tr key={item.id} className="border-b border-gray-100 hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3 text-center"><span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${categoriaColors[item.categoria] ?? 'bg-gray-100 text-gray-600'}`}>{categoriaLabels[item.categoria] ?? item.categoria}</span></td>
                        <td className="px-4 py-3 text-right"><div className="flex justify-end gap-2"><button onClick={() => navigate(`/legislacao/${item.id}`)} className="px-3 py-1.5 text-xs font-medium text-[#003087] bg-blue-50 rounded-lg hover:bg-blue-100">Editar</button><button onClick={() => handleDelete(item)} className="px-3 py-1.5 text-xs font-medium text-red-700 bg-red-50 rounded-lg hover:bg-red-100">Excluir</button></div></td>
                      </tr>
                    ))
                  )}
                </tbody>
