...
export default function QuemSomosList() {
  const navigate = useNavigate();
  const [searchTerm, setSearchTerm] = useState('');
  const [searchInput, setSearchInput] = useState('');

  const baseWhere = searchTerm ? { tituloDaPagina: { contains: searchTerm, mode: 'insensitive' as const } } : undefined;
  const { data, loading, error, total, refetch, setSkip } = useList({
    queryName: 'paginasQuemSomos',
    fields: 'id tituloDaPagina muralTextoIntrodutorio timeTitulo areaResponsavel { id nome } subarea { id nome }',
    baseWhere, take: 10, skip: 0,
  });
  const items = (data as PaginaQuemSomosItem[]) ?? [];
  ...
  const handleDelete = useCallback(async (item: PaginaQuemSomosItem) => {
    if (!window.confirm(`Excluir página "${item.tituloDaPagina}"?`)) return;
    try { await graphqlRequest(`mutation { deletePaginaQuemSomos(where: { id: "${item.id}" }) { id } }`); refetch(); }
    catch { alert('Erro ao excluir'); }
  }, [refetch]);

  return (
    <Layout title="Quem Somos - Páginas">
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <p className="text-sm text-gray-500">Total de {total} página(s)</p>
          <button onClick={() => navigate('/quem-somos/novo')} className="inline-flex items-center gap-2 px-4 py-2 bg-[#003087] text-white font-medium rounded-lg hover:bg-[#001f54] text-sm">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" /></svg> Nova Página
          </button>
        </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-200">
                    <th className="text-left px-4 py-3 text-sm font-semibold text-gray-700">Título da Página</th>
                ...
                <tbody>
                  {items.length === 0 ? (
                    <tr><td colSpan={4} className="px-4 py-8 text-center text-gray-500 text-sm">Nenhuma página encontrada.</td></tr>
                  ) : (
                    items.map((item) => (
                      <tr key={item.id} className="border-b border-gray-100 hover:bg-gray-50">
                        <td className="px-4 py-3"><p className="text-sm font-medium text-gray-900">{item.tituloDaPagina}</p></td>
                        <td className="px-4 py-3 text-sm text-gray-600">{item.areaResponsavel?.nome ?? '-'}</td>
                        <td className="px-4 py-3 text-sm text-gray-600">{item.subarea?.nome ?? '-'}</td>
                        <td className="px-4 py-3 text-right">
                          <div className="flex justify-end gap-2">
                            <button onClick={() => navigate(`/quem-somos/${item.id}`)} className="px-3 py-1.5 text-xs font-medium text-[#003087] bg-blue-50 rounded-lg hover:bg-blue-100">Editar</button>
