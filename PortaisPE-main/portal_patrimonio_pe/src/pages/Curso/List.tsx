...
function fmtDate(iso: string | null | undefined): string {
  if (!iso) return '-';
  try { const d = new Date(iso); return `${d.getDate().toString().padStart(2, '0')}/${(d.getMonth() + 1).toString().padStart(2, '0')}/${d.getFullYear()} ${d.getHours().toString().padStart(2, '0')}:${d.getMinutes().toString().padStart(2, '0')}`; }
  catch { return iso; }
}
...
export default function CursoList() {
  ...
  const { data, loading, error, total, refetch, setSkip } = useList({
    queryName: 'cursos',
    fields: 'id titulo status resumo modalidade local dataDoCurso areaResponsavel { id nome } subarea { id nome }',
    baseWhere, take: 10, skip: 0,
  });
  const items = (data as CursoItem[]) ?? [];
  ...
  const handleDelete = useCallback(async (item: CursoItem) => {
    if (!window.confirm(`Excluir "${item.titulo}"?`)) return;
    try { await graphqlRequest(`mutation { deleteCurso(where: { id: "${item.id}" }) { id } }`); refetch(); } catch { alert('Erro ao excluir'); }
  }, [refetch]);
    ...
    <Layout title="Cursos">
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <p className="text-sm text-gray-500">Total de {total} curso(s)</p>
          <button onClick={() => navigate('/cursos/novo')} className="inline-flex items-center gap-2 px-4 py-2 bg-[#003087] text-white font-medium rounded-lg hover:bg-[#001f54] text-sm">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" /></svg> Novo Curso
          </button>
        </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-200">
                    ...
                    <th className="text-center px-4 py-3 text-sm font-semibold text-gray-700">Data</th>
                    <th className="text-right px-4 py-3 text-sm font-semibold text-gray-700">Ações</th>
                  </tr>
                </thead>
                <tbody>
                  {items.length === 0 ? (
                    <tr><td colSpan={5} className="px-4 py-8 text-center text-gray-500 text-sm">Nenhum curso encontrado.</td></tr>
                  ) : (
                    items.map((item) => (
                      <tr key={item.id} className="border-b border-gray-100 hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3 text-center">
                          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${item.status === 'REALIZADO' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>{item.status === 'REALIZADO' ? 'Realizado' : 'Programado'}</span>
                        </td>
                        <td className="px-4 py-3 text-center text-sm text-gray-600 whitespace-nowrap">{fmtDate(item.dataDoCurso)}</td>
                        <td className="px-4 py-3 text-right"><div className="flex justify-end gap-2"><button onClick={() => navigate(`/cursos/${item.id}`)} className="px-3 py-1.5 text-xs font-medium text-[#003087] bg-blue-50 rounded-lg hover:bg-blue-100">Editar</button><button onClick={() => handleDelete(item)} className="px-3 py-1.5 text-xs font-medium text-red-700 bg-red-50 rounded-lg hover:bg-red-100">Excluir</button></div></td>
                      </tr>
                    ))
                  )}
                </tbody>
