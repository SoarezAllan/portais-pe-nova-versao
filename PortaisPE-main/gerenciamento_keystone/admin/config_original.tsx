...
function CleanPasteLayer() {
  useEffect(() => {
    const handlePaste = (e: ClipboardEvent) => {
      ...
      // Clean HTML comments (<!-- StartFragment --> etc.)
      const cleanHtml = html ? html.replace(/<!--[\s\S]*?-->/g, '') : '';

      if (cleanHtml) {
        // Parse cleaned HTML to extract plain text
        const parser = new DOMParser();
        const doc = parser.parseFromString(cleanHtml, 'text/html');
        // innerText normalizes whitespace better than textContent
        text = doc.body.innerText || doc.body.textContent || text;
      }

      if (!text) return;

      // Normalize whitespace to avoid extra spacing in Slate
      text = text
        .replace(/\r\n/g, '\n')
        .replace(/\n{3,}/g, '\n\n')
        .replace(/[ \t]+$/gm, '')
      ...
      // Insert clean plain text using execCommand('insertText')
  ...
  return (
    <div css={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', position: 'relative' }}>
      <div css={{ margin: '18px 18px 0' }}>
        ...
        <nav role="navigation" aria-label="NavegaÃ§Ã£o lateral" css={{ marginTop: 12 }}>
    ...
    <div>
      ...
      {isOpen && (
        ...
        >
          ...
          <ProfileLine label="FunÃ§Ã£o" value={loading ? 'Carregando' : role} />
          <ProfileLine label="Ãreas" value={areas.length ? areas.join(', ') : 'Nenhuma Ã¡rea atribuÃda'} />
        </div>
      )}
    </div>
  );
...
function CustomNavigation({ authenticatedItem, lists }: NavigationProps) {
  ...
  const { data } = useQuery(CURRENT_USER, { variables: { id: userId }, skip: !userId });
  const isAdmin = data?.user?.role === 'admin';
  const listsByKey = new Map(lists.map((list) => [list.key, list]));
  const groupedKeys = new Set(groups.flatMap((group) => group.lists));
  const ungrouped = lists.filter((list) => !groupedKeys.has(list.key));
