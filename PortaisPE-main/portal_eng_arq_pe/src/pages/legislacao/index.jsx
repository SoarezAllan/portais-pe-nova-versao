...
import { fetchLegislacoes, fetchSubareas } from "../../services/api";
...
function LegislacaoIndexPage() {
  ...
  useEffect(() => {
    const loadLegislacoes = async () => {
      try {
        setLoading(true);
        const data = await fetchLegislacoes(siteConfig.SITE_AREA);
  ...
  useEffect(() => {
    ...
    const agrupado = filtradas.reduce((acc, leg) => {
      const raw = (leg.subarea?.nome || '').toString();
              ...
              Object.entries(legislacoesAgrupadas).map(([areaKey, legis]) => (
                ...
                >
                  ...
                  <ul className="legislacao-modern-list">
                    {legis.map((leg) => (
                      <li key={leg.id} className="legislacao-modern-list-item">
                        <a
