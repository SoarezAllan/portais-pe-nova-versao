...
export default function NoticiasHistorico() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const data = await fetchNoticias(siteConfig.SITE_AREA);
        const processadas = data.map((item) => ({
          ...item,
          imagemUrl: item.imagemPrincipal?.url || null,
          imagemAlt: item.imagemPrincipalAlt || item.titulo,
        }));
  ...
  useEffect(() => {
    ...
    if (tagFilter) {
      filtradas = filtradas.filter((n) =>
        (n.tags || []).some((t) => normalize(t?.nome).includes(tagFilter))
      );
    }
    if (searchTerm) {
    ...
    <>
      ...
      <main className="page-main-content">
        <section className="noticias-section">
          <h1 className="section-title historico-title">HISTÓRICO DE NOTÍCIAS</h1>
            ...
            <div className="active-filter-container">
              <div className="filter-info">
                ...
                <span className="filter-tag-badge">
                  #{rawTagFilter}
                  <span className="filter-count">{filteredNoticias.length}</span>
                </span>
              </div>
              <button onClick={clearTag} className="clear-filter-btn">Limpar filtro ✕</button>
            </div>
          )}
          ...
          {filteredNoticias.length === 0 && !loading && (
            <div className="noticia-no-results">
              <p>
                ...
                {rawTagFilter ? ` com a tag "${rawTagFilter}"` : ""}
              </p>
              {rawTagFilter && (
                <button onClick={clearTag} className="btn-voltar-filtro">
              ...
              <div key={noticia.id} className="noticia-item">
                {noticia.imagemUrl && (
                  <div className="noticia-img-wrap">
                    <img src={noticia.imagemUrl} alt={noticia.imagemAlt} className="noticia-img" />
                  </div>
                )}
                <div className="noticia-text">
                  ...
                  <span className="noticia-data">
                    {format(new Date(noticia.dataPublicacao), "dd/MM/yyyy")}
                  </span>
                </div>
                <Link to={`/materias/${noticia.slug}`} className="noticia-btn">
                  Ler Matéria
                </Link>
              </div>
            ))}
